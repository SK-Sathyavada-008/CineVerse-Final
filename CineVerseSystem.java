...
public class CineVerseSystem {
    ...
    // File to save each user's watchlist and watch history
    private static final String LISTS_FILE  = "watchlists_history.txt";
    ...
    public CineVerseSystem() {
        ...
        loadUserListsFromFile(); // reads watchlists_history.txt – restores each user's watchlist and history
    }
    ...
    public void loadMoviesFromFile() {
        try (BufferedReader br = new BufferedReader(new FileReader(MOVIES_FILE))) {
            ...
            // Silent load – no output printed so the startup screen stays clean
        } catch (IOException e) {
    ...
    // Reads watchlists_history.txt and restores each user's watchlist and watch history
    public void loadUserListsFromFile() {
        try (BufferedReader br = new BufferedReader(new FileReader(LISTS_FILE))) {
            String line;
            User target = null;
            while ((line = br.readLine()) != null) {
                line = line.trim();
                ...
                if (line.startsWith("USER ")) {
                    // Locate the user object that was already loaded from users.txt
                    String uname = line.substring(5).trim();
                    target = null;
                ...
                } else if (line.startsWith("WATCHLIST ") && target != null) {
                    String ids = line.substring(10).trim();
                    if (!ids.isEmpty()) {
                        String[] tokens = ids.split(",");
                        // Iterate forward so the last id ends up at the head (stack order preserved)
                        // Use direct node insert – bypasses addToWatchlist() so no console output is printed on startup
                        for (int i = 0; i < tokens.length; i++) {
                            int mid = Integer.parseInt(tokens[i].trim());
                            Movie m = getMovieById(mid);
                            if (m != null) {
                                WatchNode newNode = new WatchNode(m);
                                newNode.next = target.watchlistHead;
                                target.watchlistHead = newNode;
                            }
                        }
                    }

                } else if (line.startsWith("HISTORY ") && target != null) {
                    String ids = line.substring(8).trim();
                    if (!ids.isEmpty()) {
                        String[] tokens = ids.split(",");
                        // Iterate forward so the last id ends up at the head (most-recent-first)
                        // Use direct node insert – bypasses addToHistory() so no console output is printed on startup
                        for (int i = 0; i < tokens.length; i++) {
                            int mid = Integer.parseInt(tokens[i].trim());
                            Movie m = getMovieById(mid);
                            if (m != null) {
                                WatchNode newNode = new WatchNode(m);
                                newNode.next = target.historyHead;
                                target.historyHead = newNode;
                            }
                        }
                    }
                }
            ...
            // File may not exist yet on first run – that is fine, start with empty lists
        }
    }
    ...
    public void addMovie(Scanner sc) {
        ...
        // Duplicate check – compare title case-insensitively
    ...
    public void undoWatchlistAdd() {
        ...
        // Persist updated watchlist to file
        saveUserListsToFile();
    }
    ...
    public void watchMovie(Scanner sc) throws InterruptedException {
        ...
        // Trie-inspired fallback: if exact title not found, use the first letter (String.charAt(0))
        // to collect all movies whose title starts with the same letter, then let the user pick one
        if (m == null) {
            char firstLetter = Character.toLowerCase(title.charAt(0));

            // Collect all movies starting with the same first letter
            Movie[] suggestions = new Movie[count];
            int sugCount = 0;
            for (int i = 0; i < count; i++) {
                if (Character.toLowerCase(movies[i].title.charAt(0)) == firstLetter) {
                    suggestions[sugCount++] = movies[i];
                }
            }

            // No movies share the first letter either – give up
            if (sugCount == 0) {
                System.out.println("Movie not found!");
                return;
            }

            // Show the suggestions numbered from 1
            System.out.println("\nMovie not found! Did you mean one of these? (titles starting with '" + Character.toUpperCase(firstLetter) + "')");
            System.out.println("-".repeat(120));
            for (int i = 0; i < sugCount; i++) {
                System.out.print((i + 1) + ". ");
                suggestions[i].display();
            }
            System.out.println((sugCount + 1) + ". None of these");
            System.out.print("Enter number: ");
            int pick = sc.nextInt();

            // User chose "None of these" or an out-of-range number
            if (pick == sugCount + 1 || pick < 1 || pick > sugCount) {
                System.out.println("Movie not found!");
                return;
            }

            // User picked a valid suggestion
            m = suggestions[pick - 1];
        }
        ...
        // Persist updated watchlist and history to file
        saveUserListsToFile();
    ...
    // Rewrites watchlists_history.txt with the current watchlist and history of every user.
    // Called whenever a watchlist or history changes so the file always stays in sync.
    // Format written:
    //   USER <username>
    //   WATCHLIST <id1>,<id2>,...   (empty string if none)
    //   HISTORY   <id1>,<id2>,...   (empty string if none)
    public void saveUserListsToFile() {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(LISTS_FILE, false))) { // overwrite each time
            bw.write("# CineVerse User Watchlists and Watch Histories\r\n");
            bw.write("# Format: USER <username> / WATCHLIST <ids> / HISTORY <ids>\r\n");
            bw.write("# -----------------------------------------------------------------------\r\n");
            for (int i = 0; i < userCount; i++) {
                User u = users[i];
                bw.write("USER " + u.username + "\r\n");

                // Build watchlist id string (head = most-recently-added; write tail-to-head so reload order is correct)
                StringBuilder wl = new StringBuilder();
                // Collect nodes into a temporary array to reverse the order
                WatchNode wTemp = u.watchlistHead;
                int wSize = 0;
                while (wTemp != null) { wSize++; wTemp = wTemp.next; }
                int[] wIds = new int[wSize];
                wTemp = u.watchlistHead;
                for (int k = wSize - 1; k >= 0; k--) { wIds[k] = wTemp.movie.movieId; wTemp = wTemp.next; }
                for (int k = 0; k < wSize; k++) {
                    if (k > 0) wl.append(",");
                    wl.append(wIds[k]);
                }
                bw.write("WATCHLIST " + wl.toString() + "\r\n");

                // Build history id string (same reversal so oldest entry is first in file)
                StringBuilder hs = new StringBuilder();
                WatchNode hTemp = u.historyHead;
                int hSize = 0;
                while (hTemp != null) { hSize++; hTemp = hTemp.next; }
                int[] hIds = new int[hSize];
                hTemp = u.historyHead;
                for (int k = hSize - 1; k >= 0; k--) { hIds[k] = hTemp.movie.movieId; hTemp = hTemp.next; }
                for (int k = 0; k < hSize; k++) {
                    if (k > 0) hs.append(",");
                    hs.append(hIds[k]);
                }
                bw.write("HISTORY " + hs.toString() + "\r\n");
            }
        } catch (IOException e) {
            System.out.println("Warning: Could not save watchlists/history to file. " + e.getMessage());
        }
    }

    // Helper – find a Movie object by its numeric id (used when loading lists from file)
